...
@Component({
  selector: 'app-shared-chooseoffice',
  styles:[`
  .active {
    background: #1890FF;
    color: #fff;
  ...
  `],
  templateUrl: './chooseOffice.html',
})
export class ChooseOfficeComponent implements OnInit, OnDestroy {
  query: any = {};
  ...
  selectedNode: any[] = [];
  selectedAllNode: any[] = [];
  private resourceUrl = `${API_URL}/video-ai/sysOffice`;
  ...
  ngOnInit() {
    if(this.query.officeIdString != null && this.query.officeIdString.length>0) {
      this.nzSelectedKeys = this.query.officeIdString.split(',');
    }
    this.load();
  }
  ngOnDestroy(){
  ...
  load(){
    this.loading = true;
    let body = this.query;
    this.http.post(`${this.resourceUrl}/getTree`, body,body)
    .pipe(
      map((res: any) =>{
          return res;
        }
